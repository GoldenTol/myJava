...
public class MyMap {
    public static void main(String[] args) {
        ...
        // "삼성"팀을 8위로 변경해보세요.
        map.put("삼성", 8);
        System.out.println("존재하는 key에 대한 put() 메소드 동작은 덮어쓰기(overwrite)가 됩니다.");
        // "두산" 팀을 목록에서 제외하세요.
        map.remove("두산");

        // "LG"의 순위를 확인해보세요.
        findteam="LG";
        Object rank=map.get(findteam); // .get(key)
        System.out.println(findteam+"순위 : "+rank);

        // "KT"와 "NC"의 순위를 배열을 사용하여 확인해보세요.
        String[] team={"KT", "NC"};
        for (int i = 0; i < team.length; i++) {
            rank=map.get(team[i]);
            System.out.println(team[i]+"순위 : "+rank);
        }

        // get() 메소드를 사용하여 "한화"팀이 존재하는지 확인하고, 없으면 3위로 추가하기
        findteam="한화";
        ...
        // "NC"와 "삼성"은 순위 1 빼기, "LG"는 순위 1 더하기
        keylist=map.keySet();
        for (String key:keylist){
            Integer value=map.get(key);
        ...
        System.out.println("요소 크키 : "+map.size());
    }
}
